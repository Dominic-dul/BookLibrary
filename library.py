...
def add_user():
    ...
    connection.autocommit = True
    cursor = connection.cursor()
    cursor.execute("INSERT INTO users (user_id, user_name, user_surname, user_nickname) VALUES (%s, %s, %s, %s)", (id, name, surname, nickname))
    cursor.close()
    connection.close()
...
def modify_user(user_id):
    if request.method == "GET":
        ...
        return render_template("modifyUser.html", user = single_user)
    ...
    if request.method == "POST":
        ...
        connection.autocommit = True
        cursor = connection.cursor()
        cursor.execute("UPDATE users SET user_name = %s, user_surname = %s, user_nickname = %s WHERE user_id = %s", (name, surname, nickname, user_id))
        cursor.close()
        connection.close()
...
def modify_game(game_id):
    if request.method == "GET":
        ...
        cursor = connection.cursor()
        cursor.execute("SELECT id, name FROM games WHERE id = %s;", (game_id,))
...
def add_part(game_id):
    if request.method == "GET":
        return render_template("newPart.html", game_id = game_id)
    if request.method == "POST":
        id = str(uuid.uuid4())
        name = request.form.get("name")
        quantity = int(request.form.get("quantity"))
